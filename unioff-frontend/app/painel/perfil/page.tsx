...
export default function PerfilEmpresaPage() {
  ...
  useEffect(() => {
    empresas.minha().then(setEmpresa).catch((err: Error) => setAviso({ tipo: "erro", texto: err.message }));
  }, []);
  ...
  async function aoEnviar(e: React.FormEvent<HTMLFormElement>) {
    ...
    setAviso(null);
    try {
      setEmpresa(await empresas.atualizarMinha(dados));
  ...
  if (!empresa) {
    return aviso ? <Aviso tipo="erro">{aviso.texto}</Aviso> : <p className="text-apagado">Carregando dados…</p>;
  }

  return (
    <form key={empresa.id} onSubmit={aoEnviar} className="flex max-w-xl flex-col gap-4">
      <h1 className="mb-2 font-display text-3xl font-bold">Dados do estabelecimento</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Campo rotulo="Cidade" name="cidade" defaultValue={empresa.cidade} />
        <Campo rotulo="Bairro" name="bairro" defaultValue={empresa.bairro} />
      </div>
      <div className="grid gap-4 sm:grid-cols-[1fr_7rem]">
        <Campo rotulo="Rua" name="logradouro" defaultValue={empresa.logradouro} />
        <Campo rotulo="Número" name="numero" defaultValue={empresa.numero} />
      </div>
